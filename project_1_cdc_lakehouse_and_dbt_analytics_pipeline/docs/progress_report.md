...
#### Phase A: CDC Correctness / Replay -  Spark Structured Streaming + checkpoint recovery
...
#### Phase B: Iceberg Upsert + Time Travel -  Bronze/Silver/Gold three-layer architecture
...
#### Phase C: K8s + Observability -  K8s Kustomize overlays (dev/stg/prod)
...
| 3 | Idempotency acceptance |  | Code implemented, needs runtime verification (N=3 or 5) |
...
-  1 needs runtime verification (idempotency)
